...
def index():
	...
	if current_tick >= next_tweet or test_tweet == True:
		client = get_client()
		...
		for i in range(tweet_count):
			print(f"Trying to tweet #{i}")
			tweet_quote(client, quotes)
			if tweet_count > 1:
				time.sleep(59 / tweet_count)

	returned_text = f"(at {current_tick}) I will tweet at {next_tweet} (in {next_tweet.hour - current_tick.hour - 1} hour(s) and {60 - current_tick.minute} minute(s).)"
