...
for i in range(12):
	if (last_hour + i) % tweet_hour_multiple == 0:
		if (last_hour + i) < 24:
			next_tweet = last_tick.replace(hour=last_hour+i, minute=0, second=0, microsecond=0)
			break
			
		else:
			next_tweet = last_tick.replace(hour=0,minute=0,second=0,microsecond=0) + timedelta(days=1)
			break
...
def index():
	...
	if current_tick >= next_tweet:
		...
		if current_tick.hour + tweet_hour_multiple < 24:
			last_tick = current_tick.replace(hour=current_tick.hour + tweet_hour_multiple, minute=0, second=0, microsecond=0)
			
		else:
			last_tick = current_tick.replace(hour=24 - (current_tick.hour + tweet_hour_multiple),minute=0,second=0,microsecond=0) + timedelta(days=1)
